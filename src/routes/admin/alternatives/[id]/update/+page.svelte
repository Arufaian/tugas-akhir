<script lang="ts">
	...
	let {
		...
		data: { form: SuperValidated<Infer<UpdateAlternativeSchema>> };
	} = $props();
	...
	const form = superForm(initialForm(), {
		validators: zod4Client(updateAlternativeSchema),
		onUpdated({ form: f }) {
	...
	onMount(() => {
		try {
			const imgStr = $formData.img;
			if (imgStr) {
				const parsed = JSON.parse(imgStr);
				files = [
					{
						name: parsed.originalName || 'Gambar',
						type: 'image/*',
						size: 0,
						uploadedAt: Date.now(),
						url: parsed.url,
			...
			// invalid JSON — no existing image
		}
	});
...
<Card.Root>
	<Card.Header>
		<Card.Title class="font-display text-2xl font-semibold tracking-tight"
			>Edit Alternatif</Card.Title
		>
		<Card.Description class="text-sm text-muted-foreground">
			Perbarui informasi motor yang dipilih
		</Card.Description>
	</Card.Header>
		...
		<form method="POST" use:enhance class="flex flex-col gap-6">
			...
			<Form.Field {form} name="isActive">
				<Form.Control>
					{#snippet children({ props })}
						<div class="flex items-center gap-2">
							<Switch {...props} id="isActive" bind:checked={$formData.isActive} />
							<label for="isActive" class="text-sm font-medium">Aktif</label>
						</div>
					{/snippet}
				</Form.Control>
			</Form.Field>
