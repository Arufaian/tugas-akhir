<script lang="ts">
	...
	let alternativesById = $derived(
		new Map(alternatives.map((alternative) => [alternative.id, alternative]))
	);
		...
		<Card.Root>
			...
			<Card.Content class="px-0">
				<Table.Root>
					...
					<Table.Body>
						{#each results as result (result.alternativeId)}
						...
						{#if step < 4}
							<div class="rounded-lg border">
								<Table.Root class="min-w-272">
									<Table.Caption class="sr-only">
										{currentStage.heading} untuk {alternatives.length} alternatif dan
												...
												<Table.Head class="min-w-32 text-right align-top">
													<div class="flex flex-col items-end gap-1 py-2">
														<div class="flex items-center gap-1">
															...
															<Badge variant={criterion.type === 'benefit' ? 'success' : 'warning'}>
									...
									<Table.Body>
										{#each matrixRows as row (row.alternativeId)}
											...
											{#if alternative}
												<Table.Row>
													...
													{#each criteria as criterion, criterionIndex (criterion.id)}
														<Table.Cell class="text-right font-mono tabular-nums">
															{#if step === 1}
										...
										{#each results as result (result.alternativeId)}
											...
											{#if alternative}
												<Table.Row>
													...
													<Table.Cell class="pe-4 text-right font-mono font-semibold tabular-nums">
														{formatDecimal(result.optimizationScore)}
													</Table.Cell>
												</Table.Row>
											{/if}
